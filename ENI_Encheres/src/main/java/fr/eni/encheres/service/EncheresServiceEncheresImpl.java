...
public class EncheresServiceEncheresImpl implements EncheresServiceEncheres {
	...
	public String messageBoutonEncherirIndisponible(ArticleVendu article, Principal user) {
		...
		if (user == null)
			return "Connectez-vous pour enchérir sur cet article";
		if (!verificationVendeur(article, user))
			return "Vous êtes le vendeur de cet article";
		if (!verificationDatesEnchereEnCours(article))
			return "Cette enchère n'est pas en cours";
		if (!verificationDernierEncherisseur(article, user))
			return "Vous avez déjà enchérit sur cet article !";
		if (!verificationSoldeSuffisant(article, user))
			return "Vous n'avez pas assez de crédits pour enchérir sur ce produit";
		return null;
	}
	...
	private Boolean verificationMeilleurEncherisseur(ArticleVendu article, Principal user) {
		...
		if (getMeilleureEnchereByArticle(article) == null)
			return false;
		if (encheresServiceUtilisateur.findUserByPseudo(user.getName())
				.getNo_utilisateur() == getMeilleureEnchereByArticle(article).getUtilisateur().getNo_utilisateur()) {
			return true;
		}
		return false;
	}
	...
	private Boolean verificationDatesEnchereEnCours(ArticleVendu article) {
		...
		if ((datedujour.isAfter(article.getDate_debut_encheres())
				&& (datedujour.isBefore(article.getDate_fin_encheres())))) {
			return true;
		}
		return false;
	}
	...
	private Boolean verificationVendeur(ArticleVendu article, Principal user) {
		...
		if (article.getUtilisateur().getNo_utilisateur() == encheresServiceUtilisateur.findUserByPseudo(user.getName())
				.getNo_utilisateur()) {
			return false;
		}
		return true;
	...
	public void surencherir(Enchere enchere) {
		...
		if (ancienneMeilleureEnchere != null) {
			encheresServiceUtilisateur.miseAJourCredit(ancienneMeilleureEnchere.getMontant_enchere(),
					ancienneMeilleureEnchere.getUtilisateur());
		}
